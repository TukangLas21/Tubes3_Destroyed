...
def create_summary_content(detail_id:int, applicant_id: int):
    # proses detail_id dan applicant id menjadi data cv
    summary_text = "" # parsing data ke sini
    summary_content = ft.Container(
        ft.Column([
            # Title + button to go back
            ft.Container(
            ...
            ft.Container(
                ft.Column(
                    spacing = 8,
                    controls = [
                        ft.Text(summary_text, style = BODY2_SECONDARY_STYLE),
                    ],
                    width = 650,
                ),
                bgcolor = APP_COLORS["glass"],
